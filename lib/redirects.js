...
RedirectableRequest.prototype.setTimeout = function h(timeout, cb) {
  this._currentRequest.setTimeout(timeout, cb);
};

RedirectableRequest.prototype._write = function i(data, encoding, cb) {
  this._currentRequest.write(data, encoding, cb);
  this._bufferedWrites.push({ data: data, encoding: encoding });
};

RedirectableRequest.prototype.end = function j(data, encoding, cb) {
...
(0, _keys2.default)(nativeProtocols).forEach(function (protocol) {
  ...
  var nativeProtocol = nativeProtocols[protocol];
  var wrappedProtocol = module.exports[scheme] = (0, _create2.default)(nativeProtocol);

  wrappedProtocol.request = function k(options, cb) {
    var newOptions = void 0;
    if (typeof options === 'string') {
      newOptions = (0, _assign2.default)({}, url.parse(options), { maxRedirects: 21 });
    } else {
  ...
  wrappedProtocol.get = function l(options, cb) {
    var request = wrappedProtocol.request(options, cb);
    request.end();
    return request;
  };
});